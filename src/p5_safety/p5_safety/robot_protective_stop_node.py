...
class RobotProtectiveStop(Node):
    def __init__(self):
        super().__init__('p5_robot_protective_stop')
        ...
        self.pose_sub_a = self.create_subscription(
            PoseStamped,
            '/alice/p5_robot_pose_before_safety',
            self.pose_alice_callback,
            10)

        self.pose_sub_b = self.create_subscription(
            PoseStamped,
            '/bob/p5_robot_pose_before_safety',
        ...
        self.pose_pub_alice = self.create_publisher(PoseStamped, '/alice/servo_node/pose_target_cmds', 10)
        self.pose_pub_bob = self.create_publisher(PoseStamped, '/bob/servo_node/pose_target_cmds', 10)
    ...
    def _get_tf_tree_pose(self, parent_name, child_name):
        try:
            ...
            self.error_handler.report_error(self.error_handler.info,
                                            f'Failed to get transform from {parent_name}/{child_name}: {e}')
